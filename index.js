...
proto.listen = function(port, cb) {
    ...
    // support for tcp
    this.tcp = require( 'net' ).createServer( (c) => {
      c.on('data', (msg) => {
        var info = parser(msg, {address: this.tcp.address, family: this.tcp.family, port: this.tcp.port, size: msg.length});
        me.handler(info);
      });
      c.on('err', (err) => {
	this.emit('error', err);
